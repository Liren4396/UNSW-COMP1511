...
int encrypt(int character, char* normal_order) 
{
    int shift = 0;
    char order[27] = "abcdefghijklmnopqrstuvwxyz";
    for (int i = 0; i < 27; i++)
    {
...
int main()
{
    char normal_order[27];
    printf("Enter mapping:\n");
    scanf("%s", normal_order);
    getchar();
    char word;
    printf("Enter text:\n");
    while (scanf("%c", &word) != EOF)
    {
        if (word != ' ')
        {
            word = encrypt(word, normal_order);
            printf("%c", word);
        }
        else
        {
